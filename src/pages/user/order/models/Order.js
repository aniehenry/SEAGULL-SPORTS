class Order {
  constructor(data = {}) {
    ...
    // Enhanced address fields from checkout stage 1
    this.deliveryAddress = data.deliveryAddress || null;
    this.customerName = data.customerName || "";
    this.customerEmail = data.customerEmail || "";

    // Contact details from checkout stage 2
    this.customerPhone = data.customerPhone || "";
    this.alternatePhone = data.alternatePhone || "";

    // Payment information from checkout stage 2
  ...
  toFirestore() {
    ...
    return {
      ...
      // Enhanced address/customer info from checkout stage 1
  ...
  static fromCheckoutData(checkoutData, cartItems, userId) {
    return new Order({
      ...
      // Address information (stage 1)
      deliveryAddress: checkoutData.address,
      customerName: checkoutData.address?.fullName || "",
      customerEmail: checkoutData.address?.email || "",

      // Contact information (stage 2)
      customerPhone: checkoutData.contact?.phoneNumber || "",
      alternatePhone: checkoutData.contact?.alternatePhone || "",

      // Payment information (stage 2)
      ...
      paymentDetails:
        checkoutData.payment?.method === "cod"
          ...
          : {
              ...
              // Note: Never store actual card details for security
              cardLast4:
                checkoutData.payment?.cardDetails?.cardNumber?.slice(-4) || "",
              cardType: checkoutData.payment?.method,
            },
      ...
      status: "pending",
      paymentStatus:
        checkoutData.payment?.method === "cod" ? "pending" : "pending",
      orderDate: new Date(),
    });
  }
